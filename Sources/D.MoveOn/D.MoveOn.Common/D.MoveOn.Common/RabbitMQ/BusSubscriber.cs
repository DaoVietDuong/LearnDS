...
namespace D.MoveOn.Common.RabbitMQ
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, InternalTicketException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is InternalTicketException internalTicketException && onError != null)
                        {
                            var rejectedEvent = onError(message, internalTicketException);
